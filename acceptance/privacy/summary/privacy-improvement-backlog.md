...
| 编号 | 状态 | 说明 |
| --- | --- | --- |
| PRIV-P1-001 | 已修复 | 已补敏感个人信息判断和默认升级条件。 |
| PRIV-P1-002 | 已修复 | 已补角色判断表，覆盖个人信息处理者、受托处理者、共同处理者、第三方接收方。 |
| PRIV-P1-003 | 已修复 | 已补剩余风险等级、责任部门、整改期限和风险接受人角色。 |
| PRIV-P2-001 | 已缓解 | 已拆短处理活动台账；CSV 阅读体验保留为后续优化项。 |
| PRIV-P2-002 | 已修复 | 已按请求类型拆分答复提纲。 |
| PRIV-P2-003 | 已修复 | 已补远程运维、境外客服、集团内部访问和境外 SaaS 场景清单。 |

## Alpha 修复后发布判断

- P0：0
- P1：0
- P2：1，保留为不影响 alpha 试用的体验优化项。

可建议发布 `v0.4.0-alpha`，前提是 main 验证脚本全部通过，且未发现 `.env`、私钥、密钥文件或 `practice-profile.md`。
