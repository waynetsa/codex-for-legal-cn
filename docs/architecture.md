...
当前项目仍是公开模板库，不是生产系统。仓库内只应存放插件、模板、检查清单、虚构样例、脱敏样例和说明文档；不得接入真实律所系统、文档系统、合同系统、案件系统、法研数据库、监管数据库或真实 MCP。

私有化部署前必须先完成访问控制、用户权限、matter 隔离、客户隔离、日志、审计、数据留存、删除策略、模板审批和输出复核设计。详见 [公开仓库与私有部署边界](public-vs-private-deployment-boundary.md)。
